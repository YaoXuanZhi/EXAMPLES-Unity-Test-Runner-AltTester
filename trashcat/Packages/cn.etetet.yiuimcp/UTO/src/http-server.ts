 ...
 * 检查连接是否因域重载而断开
 */
function isConnectionDroppedError(errorMessage: string): boolean {
    ...
    try {
        ...
        if (tool === "WaitForTestResult" && !resultText.includes("已完成")) {
            console.log(`[${timestamp}] [HTTP] WaitForTestResult 进入 UTO 层轮询...`);
                const wftTimeout = ((params as any)?.timeoutSeconds || 200) * 1000;
                ...
                while (Date.now() - pollStartTime < wftTimeout) {
                    try {
                        ...
                        const pollResult = await mcpClient.callTool("WaitForTestResult", {});
                        if (pollResult && pollResult.content && Array.isArray(pollResult.content)) {
                            const pollText = pollResult.content.map((c: any) => c.text).join('\n');
                ...
                return res.json({
                    success: true, result: resultText || `轮询超时（${((params as any)?.timeoutSeconds || 200)}秒）`,
        ...
        if ((connDropped || domainReloadDetected) && heartbeatManager) {
            ...
            if (tool === "RunTestRunner") {
                ...
                try {
                    const checkResult = await mcpClient.callTool("GetTestResult", { testMode: (params as any)?.testMode || "PlayMode" });
                    if (checkResult && checkResult.content && Array.isArray(checkResult.content)) {
                        const checkText = checkResult.content.map((c: any) => c.text).join('\n');
                ...
                try {
                    const resumeResult = await mcpClient.callTool("WaitForTestResult", {});
                    if (resumeResult && resumeResult.content && Array.isArray(resumeResult.content)) {
                        resultText = resumeResult.content.map((c: any) => c.text).join('\n');
                    }
                    isError = false;
                } catch {
...
app.get('/tools', async (req, res) => {
    try {
        const result = await mcpClient.callTool("ListTools", {});
